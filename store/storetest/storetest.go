...
// a missing picker returns, what happens when two workers create the same
...
func Run(t *testing.T, newStore Factory) {
	t.Helper()
	t.Run("MissingPickerIsNotFound", func(t *testing.T) { missingIsNotFound(t, newStore) })
	...
	t.Run("UpdateOfMissingPickerIsNotFound", func(t *testing.T) { updateMissing(t, newStore) })
	t.Run("EnabledPickersExcludesDisabled", func(t *testing.T) { enabledExcludesDisabled(t, newStore) })
...
func newPicker(id string) store.NewPicker {
	return store.NewPicker{DirectoryID: id, Login: id + "@separador.app", DisplayName: "Test " + id}
}

func missingIsNotFound(t *testing.T, newStore Factory) {
	s := newStore(t)
	_, err := s.PickerByDirectoryID(context.Background(), "hlp_nobody")
...
func createThenRead(t *testing.T, newStore Factory) {
	...
	created, err := s.CreatePicker(ctx, newPicker("hlp_a"))
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	if created.ID == "" {
		t.Fatal("CreatePicker must return the assigned id: it is written back to the directory as externalId")
	}
	if !created.Enabled {
		t.Error("a newly created picker must be enabled: it is only created for an active identity")
	}

	got, err := s.PickerByDirectoryID(ctx, "hlp_a")
	if err != nil {
...
func duplicateRejected(t *testing.T, newStore Factory) {
	...
	if _, err := s.CreatePicker(ctx, newPicker("hlp_dup")); err != nil {
		t.Fatalf("first create: %v", err)
	}
	_, err := s.CreatePicker(ctx, newPicker("hlp_dup"))
...
func concurrentCreate(t *testing.T, newStore Factory) {
	...
	for i := 0; i < workers; i++ {
		...
		go func() {
			defer wg.Done()
			p, err := s.CreatePicker(ctx, newPicker("hlp_race"))
			mu.Lock()
			defer mu.Unlock()
			switch {
			case err == nil:
	...
	if wins != 1 {
		t.Fatalf("%d concurrent creates succeeded, want exactly 1 — two workers must never "+
			"mint two pickers for one identity", wins)
	}
	if len(ids) != 1 {
...
func updateApplies(t *testing.T, newStore Factory) {
	...
	p, err := s.CreatePicker(ctx, newPicker("hlp_upd"))
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	err = s.UpdatePicker(ctx, p.ID, store.PickerUpdate{
	...
	got, err := s.PickerByDirectoryID(ctx, "hlp_upd")
	if err != nil {
		t.Fatalf("read back: %v", err)
	}
	if got.Enabled {
...
func updateMissing(t *testing.T, newStore Factory) {
	s := newStore(t)
	err := s.UpdatePicker(context.Background(), "no-such-id", store.PickerUpdate{Enabled: true})
	if !errors.Is(err, store.ErrNotFound) {
...
func enabledExcludesDisabled(t *testing.T, newStore Factory) {
	...
	on, err := s.CreatePicker(ctx, newPicker("hlp_on"))
	if err != nil {
		t.Fatal(err)
	}
	off, err := s.CreatePicker(ctx, newPicker("hlp_off"))
	if err != nil {
		t.Fatal(err)
	}
	if err := s.UpdatePicker(ctx, off.ID, store.PickerUpdate{
		Enabled: false, DisplayName: "Test hlp_off", Login: "hlp_off@separador.app",
	}); err != nil {
		t.Fatal(err)
	}

	list, err := s.EnabledPickers(ctx)
	if err != nil {
		t.Fatalf("enabled: %v", err)
	}
	if len(list) != 1 || list[0].ID != on.ID {
		t.Fatalf("EnabledPickers = %+v, want only %s. The daily walk compares this list "+
